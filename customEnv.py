...
class CustomCuttingStockEnv(CuttingStockEnv):
    def reset(self, seed=None, options=None):
        # Gọi phương thức reset của lớp cha để thiết lập môi trường ban đầu
        if(options == None ):
            return super().reset(seed=seed)    
        # Nếu options được cung cấp và chứa 'stocks', sử dụng dữ liệu tùy chỉnh
        ...
        if options and 'stocks' in options:
            ...
            # Xáo trộn danh sách self._stocks
            random.shuffle(self._stocks)    
            self._stocks = tuple(self._stocks)
            self.num_stocks = len(self._stocks)
        
        # Nếu options được cung cấp và chứa 'products', sử dụng dữ liệu tùy chỉnh
